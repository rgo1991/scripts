...
SOURCE=$1
CACHE_FILES=`grep $1 rn4/mamacache_* | awk -F: '{print $1}' | sort -u`
...
#	grep $SOURCE rn4/mamacache_source.xml | grep Value |awk -F. '{print $3,$4,$5}'
	...
	cache_host=( $(grep -w $j proc.conf.ny2| grep -v '#' |awk -F: '{print $1}') )
	
	case $SOURCE in
	AMEX) echo $j is on hosts ${cache_host[0]} and ${cache_host[1]} and ${cache_host[2]}| egrep "amex"  ;; 
	NYSE) echo $j is on hosts ${cache_host[@]}| grep 'nyse'| grep 'pny'  ;;
...
cd rn5/
FEED_NAMES=`grep -A1 WmwSource *.xml | grep -w $SOURCE | sort -u | awk -F. '{print $1}'`
...
for g in $FEED_NAMES;do
	...
	grep "$g" proc.conf.ny2| egrep -v "pny2sfmc|\#" | tr '|' "\n" |tr ':' "\n" |egrep "$g|pny2sffhr"| cut -d@ -f1 | sed s/':nsd'/" "/gi | column -t;done
	#echo "$feed_hosts";done
...
FEED=$1
cd rn5/
FEED_FILES=`grep $FEED * | awk -F: '{print$1}'| grep -v old |sort -u`
