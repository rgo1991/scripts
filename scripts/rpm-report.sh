...
# takes everything after the last '-' and everything after the second last '-'. eg: rpm=rpm-123-321, echo ${rpm%-*-*} will give back just 'rpm'.
rpmnames(){

for rpm in `cat $* | sort -u`;do
	echo ${rpm%-*-*}
	done | sort -u
...
for node in $*;do
	while read rpm;do				# rpm name is gnome-panel-2.16.1-7.el5
		...
		if [ -z "${version[$idx]}" ];then
			version[$idx]="${rpmversion}-${rpmrelease}";else
			version[$idx]="${version[$idx]} ${rpmversion}-${rpmrelease}";fi
...
for rpmname in `rpmnames $*`;do
	...
	echo "<tr class=\"same\">" >> $HTML;else
	
	echo RPM $rpmname
	echo "<tr class=\"notsame\">" >> $HTML;fi
